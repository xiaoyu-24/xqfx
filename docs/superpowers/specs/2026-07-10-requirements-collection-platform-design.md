...
12. 在需求列表下提供管理需求模块，集中跟进所有尚未完成、关闭或拒绝的需求及草稿。
...
- 需求填写时间在列表、详情和管理需求模块中统一按 `Asia/Shanghai` 显示。
- 管理需求模块包含正式需求和暂存草稿；正式需求状态为已完成、已关闭或已拒绝后移出模块。
...
- 详情页和管理需求模块使用与需求列表相同的时间格式和时区，避免同一条记录显示不一致。
...
- 当状态改为已完成、已关闭或已拒绝时，保存成功后该记录立即从管理需求模块移除，但仍可在普通需求列表中按条件查询。
- 状态改回待评估、已确认、开发中或暂停时，记录重新进入管理需求模块。
...
页面顶部显示当前页面名称。填写页和详情页提供明显的“返回”按钮。
...
- 新填写或新暂存的需求排在上方。
- 点击记录进入处理详情，支持修改状态、填写完成时间、处理人和完成情况。
- 状态保存为已完成、已关闭或已拒绝后，管理需求模块自动刷新并移除该记录。
...
| GET | `/api/requirements/management` | 查询管理需求模块中的待处理需求和草稿 |
...
- 需求分页列表和组合筛选。
- 需求列表首次打开自动加载全部需求。
- 列表、详情和管理模块统一使用上海时间展示填写时间。
- 类型、系统版本和时间周期筛选。
- 详情、编辑、状态更新和软删除。
- 需求列表下方增加管理需求模块，覆盖正式待处理需求和暂存草稿。
...
- 管理需求模块包含未完成正式需求和暂存草稿，可填写完成时间、处理人和完成情况。
- 详情、编辑、删除。
- 系统新增、改名、启停、删除保护。
- 系统负责人、多协助人和版本管理。
- 附件上传与下载。
...
- 管理需求模块准确包含正式未完成需求和暂存草稿；状态变为已完成、已关闭或已拒绝后自动移除。
...
- 填写时间在列表、详情和管理需求模块统一按上海时间展示。
- 管理需求包含正式未完成需求和暂存草稿；已完成、已关闭、已拒绝的正式需求不再展示。
- 管理需求处理记录包含完成时间、处理人和完成情况。
- 填写需求、需求列表和系统管理三张页面主体结构草图已确认；本次新增字段嵌入既有结构。

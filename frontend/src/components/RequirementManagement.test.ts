...
describe('RequirementManagement', () => {
  ...
  it('updates requirement processing details from the management page', async () => {
    ...
    await flushPromises()
    expect(wrapper.get('[data-test="processing-field-legend"]').text()).toContain('带 * 的项目为必填项')
    expect(wrapper.get('[data-test="processing-status-field"]').text()).toContain('* 必填')
    expect(wrapper.get('[data-test="processing-description-field"]').text()).toContain('选填')

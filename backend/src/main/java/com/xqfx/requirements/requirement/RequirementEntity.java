...
class RequirementEntity {
    ...
    @Column(length = 10000) private String completionDescription;
    ...
    Long id(){return id;} long recordVersion(){return recordVersion;} String requesterName(){return requesterName;} String department(){return department;} String title(){return title;} String content(){return content;} SystemEntity system(){return system;} RequirementType type(){return type;} RequirementStatus status(){return status;} RequirementSaveType saveType(){return saveType;} LocalDateTime createdAt(){return createdAt;} LocalDateTime updatedAt(){return updatedAt;} LocalDateTime submittedAt(){return submittedAt;} LocalDateTime statusUpdatedAt(){return statusUpdatedAt;} LocalDateTime completedAt(){return completedAt;} String handledBy(){return handledBy;} String completionDescription(){return completionDescription;} SystemVersionEntity targetVersion(){return targetVersion;} LocalDate periodStartDate(){return periodStartDate;} LocalDate periodEndDate(){return periodEndDate;}
    void updateStatus(RequirementStatus status){this.status=status;this.statusUpdatedAt=now();}
    void updateProcessing(RequirementStatus status,LocalDateTime completedAt,String handledBy,String completionDescription){this.status=status;this.completedAt=completedAt;this.handledBy=handledBy;this.completionDescription=completionDescription;this.statusUpdatedAt=now();}

...
class RequirementService {
 ...
 @Transactional(readOnly=true) RequirementPageResponse page(int page,int size){if(page<0||size<1||size>100)throw new IllegalArgumentException("分页参数无效");return RequirementPageResponse.from(requirements.findAllByDeletedFalse(PageRequest.of(page,size,Sort.by(Sort.Direction.DESC,"createdAt"))));}

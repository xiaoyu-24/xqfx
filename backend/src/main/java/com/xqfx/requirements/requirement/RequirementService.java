...
class RequirementService {
 ...
 @Transactional(readOnly=true) java.util.List<RequirementResponse> list(RequirementType type,Long systemId){var items=systemId!=null?requirements.findBySystemIdAndDeletedFalse(systemId):(type!=null?requirements.findByTypeAndDeletedFalse(type):requirements.findAllByDeletedFalse());return items.stream().map(RequirementResponse::from).toList();}

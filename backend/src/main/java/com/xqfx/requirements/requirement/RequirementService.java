...
class RequirementService {
 ...
 @Transactional(readOnly=true) RequirementPageResponse page(int page,int size,Long systemId,Long targetVersionId,String department,String requesterName,RequirementType type,RequirementStatus status,RequirementSaveType saveType,String keyword,LocalDate submittedFrom,LocalDate submittedTo){if(page<0||size<1||size>100)throw new IllegalArgumentException("分页参数无效");if(submittedFrom!=null&&submittedTo!=null&&submittedTo.isBefore(submittedFrom))throw new IllegalArgumentException("填写结束日期不能早于开始日期");return RequirementPageResponse.from(requirements.findAll(RequirementSpecifications.filtered(systemId,targetVersionId,department,requesterName,type,status,saveType,keyword,submittedFrom,submittedTo),PageRequest.of(page,size,Sort.by(Sort.Direction.DESC,"createdAt"))));}
